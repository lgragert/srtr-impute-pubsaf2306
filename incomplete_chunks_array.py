###############################################################################
#   SCRIPT NAME:    incomplete_chunks_array.py
#   DESCRIPTION:    Rerun incomplete chunks from calling the API for Eplet Registry
#   INPUT:          ./SRTR_output_API_{ARRAY_ID}/incomplete_chunks.json
#   OUTPUT:         ./SRTR_output_API_{ARRAY_ID}/eplet_output_chunk_{chunk_number}.csv
#   NOTES:
###############################################################################
...
input_file_path = f'./SRTR_AA_MM_9loc_matrix_{ARRAY_ID}.txt.gz'
output_dir = f'./SRTR_output_API_{ARRAY_ID}/'
...
def process_chunk(chunk, chunk_number):
    eplet_list = []
    # 1st Error handling for chunk processing (success/failure)
    try:
      for index, row in chunk.iterrows():
          ...
          try:
              ...
              DONOR_DRB1_1 = row["DONOR_DRB345_1"] # account for updated SRTR DRB groups
              DONOR_DRB1_2 = row["DONOR_DRB345_2"] # account for updated SRTR DRB groups
              DONOR_DRW1_1 = row["DONOR_DRB1_1"] # account for updated SRTR DRB groups
              DONOR_DRW1_2 = row["DONOR_DRB1_2"] # account for updated SRTR DRB groups
              ...
              RECIP_DRB1_1 = row["RECIP_DRB345_1"] # account for updated SRTR DRB groups
              RECIP_DRB1_2 = row["RECIP_DRB345_2"] # account for updated SRTR DRB groups
              RECIP_DRW1_1 = row["RECIP_DRB1_1"] # account for updated SRTR DRB groups
              RECIP_DRW1_2 = row["RECIP_DRB1_2"] # account for updated SRTR DRB groups
...
#    else:
#      print(f"First pass: failed to process chunk {chunk_number}, logging for further review.")
#      time.sleep(sleep_time)

# EDIT: Indicate which run # to log errors
# Function to log errors to a file
def log_error(message):
    with open(f"{output_dir}error_log_run2.txt", "a") as log_file:
...
def rerun_incomplete_chunks(incomplete_chunks_file):
    ...
    for chunk_number in incomplete_chunks:
        ...
        chunk = pd.read_csv(input_file_path, sep='\t', compression='gzip', skiprows=range(1, skip_rows), nrows=chunk_size, header=0)
        ...
        if process_chunk(chunk, chunk_number):
          ...
          print(f"Failed to process chunk {chunk_number} again, logging for further review and sleeping for {sleep_time} seconds...")
          time.sleep(sleep_time)
...
print(f"Incomplete chunks (2nd run): {incomplete_chunks}")
print(f"Failed chunks (2nd run): {failed_chunks}")

# Saving the list of incomplete/failed chunks
# EDIT: Indicate which run # of incomplete/failed chunks
with open(f"{output_dir}incomplete_chunks_run2.json", "w") as f:
    json.dump(incomplete_chunks, f)
with open(f"{output_dir}failed_chunks_run2.json", "w") as f:
