...
# for loc in aa_mm.ard_start_pos:
    ...
    # extract data from file HLA Eplet Registry files

    if loc in ['A','B','C']:
        eplet_filename = "Eplet_Registry_ABC.txt"
    elif loc in ['DRB1','DRB3','DRB4','DRB5']:
        eplet_filename = "Eplet_Registry_DRB.txt"
    elif loc in ["DQA1","DQB1"]:
        eplet_filename = "Eplet_Registry_DQ.txt"
    elif loc in ["DPA1","DPB1"]:
        eplet_filename = "Eplet_Registry_DP.txt"
    else:
        print ("Invalid Locus: " + loc)

    epitope = pd.read_csv(eplet_filename,sep='\t')
