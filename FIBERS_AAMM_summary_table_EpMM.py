...
for pop in pops:
    ...
    #############################
    # Filenames 
    excel_spreadsheet_filename = f'FIBERS_SRTR_AAMM_{bin_name}_{popcode}_summary_table_v6.xlsx'
    frequency_datatable_filename = f'FIBERS_SRTR_AAMM_{bin_name}_freq_data_table_v6.xlsx'
    frequency_distribution_filename = f'FIBERS_SRTR_AAMM_{bin_name}_{popcode}_freq_distr_plot_v6.png'
    #############################
...
for pop in pops:
    ...
    # Load the workbook
    print(f"Loading workbook for population {popcode}...")
    wb = load_workbook(filename=excel_spreadsheet_filename)
    print(f"Workbook for population {popcode} loaded.")
    ...
    for AA in AAMM_tabs:
        ...
        if sheet_name in wb.sheetnames:
            ...
            # Get eplet counts for AAMM sheet (similar to above)
            highcount_highgroup_aamm = []
            medcount_highgroup_aamm = []
            lowcount_highgroup_aamm = []
    ...
    # Save the workbook
    print(f"Saving workbook for population {popcode}...")
    wb.save(filename=excel_spreadsheet_filename)
    print(f"Workbook for population {popcode} saved.")
