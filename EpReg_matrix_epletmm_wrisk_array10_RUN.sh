#!/bin/bash
#SBATCH --job-name=EpReg_matrix_epletmm_wrisk_array10                    ### Job Name
#SBATCH --array=1-10                                                     ### Number of array tasks
#SBATCH --output=./logs/EpReg_matrix_epletmm_wrisk_array10_%A_%a.out     ### File in which to store job output (based on array task)
#SBATCH --error=./logs/EpReg_matrix_epletmm_wrisk_array10_%A_%a.err      ### File in which to store job error messages (based on array task)
...
python3 EpReg_matrix_epletmm_wrisk_array10.py
