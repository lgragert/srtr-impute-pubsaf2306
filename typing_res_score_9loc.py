...
# Separate out the donors from the recips by population before TRS
for pop in pops:
    impute_outfilename = pathloc + "/impute.srtr." + pop + ".csv.gz"
    print(impute_outfilename)
    impute_outfile = pd.read_csv(impute_outfilename, header=None)

    recip = impute_outfile[impute_outfile.loc[:,0].str.startswith("R")]
    donor = impute_outfile[impute_outfile.loc[:, 0].str.startswith("D")]

    recip.to_csv('impute.srtr.' + pop + '.recip.csv.gz', header=False, index=False)
    donor.to_csv('impute.srtr.' + pop + '.donor.csv.gz', header=False, index=False)
...
# Separate out by recip and donor, do the same thing
for pop in pops:
    ...
    for DR in patient:
        ...
        print(DR)
        print(pop)
        impute_outfilename = pathloc + "/impute.srtr." + pop + "." + DR + ".csv.gz"
        print(impute_outfilename)
        impute_outfile = gzip.open(impute_outfilename, "rt")
        ...
        Rtrs_dataframe = pd.DataFrame(Rtrs)
        design_matrix_filename = pathloc + "/trs_9loc_matrix_recip.csv"
        Rtrs_dataframe.to_csv(design_matrix_filename, index=False, sep=",")
        ...
        if (DR == "donor"):
            for line in impute_outfile:
                ...
                Dhappair_id_total[subject_id] += float(happair_freq)

            impute_outfile.close()

            ndonor = len(Dhappair_id_total)
            ...
            for line in impute_outfile:
                ...
                prob = happair_freq / Dhappair_id_total[subject_id]
        ...
        trs_dataframe = pd.DataFrame(Dtrs)
        design_matrix_filename = pathloc + "/trs_9loc_matrix_donor.csv"
        trs_dataframe.to_csv(design_matrix_filename, index=False, sep=",")
