...
AA = aa_mm.getAAposition(allele1,6)
...
impute_outfilename = pathloc + "/impute_all_ethnicitys.csv"
...
for line in impute_outfile:
	(cohort_id,subject_id,hap1,hap2,freq) = line.split(',')
...
#tx_ki_filename = "/Users/lgragert/Box Sync/Malek_Outcome/kamoun_impute/tx_ki.txt"
tx_ki_filename = pathloc + "/kamoun_impute/tx_ki.txt"
...
for rep in tqdm(range(1,multiple_imputation_replicates+1)):
	...
	tx_hla_filename = pathloc + "/kamoun_impute/tx_ki_hla.csv"
	...
	for line in tx_hla_file:
		(ORG_TY,PERS_ID,PX_ID,REC_TX_DT,REC_HISTO_TX_ID,DON_TY,DON_RACE,DON_RACE_SRTR,DON_ETHNICITY_SRTR,DON_A1,DON_A2,DON_B1,DON_B2,DON_DR1,DON_DR2,REC_AGE_IN_MONTHS_AT_TX,CAN_RACE,CAN_RACE_SRTR,CAN_ETHNICITY_SRTR,REC_TX_TY,REC_A1,REC_A2,REC_B1,REC_B2,REC_DR1,REC_DR2,DONOR_ID,DON_C1,DON_C2,DON_DQ1,DON_DQ2,DON_DP1,DON_DP2,DON_DR51,DON_DR52,DON_DR53,REC_CW1,REC_CW2,REC_DPW1,REC_DPW2,REC_DQW1,REC_DQW2,REC_DRW51,REC_DRW52,REC_DRW53) = line.split(',')
		if (ORG_TY == "ORG_TY"):
			continue
		...
		haplist = happair_hla[SUBJECT_ID_RECIP]
		problist = happair_probs[SUBJECT_ID_RECIP]

		happair_recip = aa_mm.weighted_choice(haplist,problist)
		# print ("Random Recip HapPair: " + PERS_ID + " " + happair_recip)

		haplist = happair_hla[SUBJECT_ID_DONOR]
		problist = happair_probs[SUBJECT_ID_DONOR]

		happair_donor = aa_mm.weighted_choice(haplist,problist)
		# print ("Random Donor HapPair: " + DONOR_ID + " " + happair_donor)

		(hap1_donor,hap2_donor) = happair_donor.split('+')
		(a1_donor,c1_donor,b1_donor,drb1_1_donor,dqb1_1_donor) = hap1_donor.split('~')
		(a2_donor,c2_donor,b2_donor,drb1_2_donor,dqb1_2_donor) = hap2_donor.split('~')

		(hap1_recip,hap2_recip) = happair_recip.split('+')
		(a1_recip,c1_recip,b1_recip,drb1_1_recip,dqb1_1_recip) = hap1_recip.split('~')
		(a2_recip,c2_recip,b2_recip,drb1_2_recip,dqb1_2_recip) = hap2_recip.split('~')
		...
		hap1_donor_alleles = [a1_donor,c1_donor,b1_donor,drb1_1_donor,dqb1_1_donor]
		hap1_donor = '~'.join(hap1_donor_alleles)
		hap2_donor_alleles = [a2_donor,c2_donor,b2_donor,drb1_2_donor,dqb1_2_donor]
		hap2_donor = '~'.join(hap2_donor_alleles)
		happair_donor = hap1_donor + "+" + hap2_donor
		hap1_recip_alleles = [a1_recip,c1_recip,b1_recip,drb1_1_recip,dqb1_1_recip]
		hap1_recip = '~'.join(hap1_recip_alleles)
		hap2_recip_alleles = [a2_recip,c2_recip,b2_recip,drb1_2_recip,dqb1_2_recip]
		...
		for loc in loci:
			...
			ard_start = aa_mm.ard_start_pos[loc]
			ard_end = aa_mm.ard_end_pos[loc]
			...
			for pos in range(ard_start,ard_end):
				...
				mm_count = aa_mm.count_AA_Mismatches_Allele(allele1_donor,allele2_donor,allele1_recip,allele2_recip,pos)
				...
				runmatch_file.write(PX_ID + "|" + loc + "|" + str(pos) + "|" + 
					str(mm_count_0) + "|" + str(mm_count_1) + "|" + str(mm_count_2) + "\n")
	...
	#print (happair_selected_recip_df["HAPPAIR_RECIP"])
	...
	for i in range(1,loci_range_value):
		...
		if loc in ['DRB3', 'DRB4', 'DRB5']:
			loc = 'DRB345'
			ard_start = aa_mm.ard_start_pos['DRB3']
			ard_end = aa_mm.ard_end_pos['DRB3']
		else:
			ard_start = aa_mm.ard_start_pos[loc]
			ard_end = aa_mm.ard_end_pos[loc]		
		...
		for id in range(1,1000):
			...
			for index,row in happair_selected.iterrows():
				mm_pos_count = aa_mm.count_AA_Mismatches_SFVT(donor_allele1[index],donor_allele2[index],recip_allele1[index],recip_allele2[index],positions)
