#!/bin/bash
#SBATCH --job-name=hla_epitope_registry_eplet_test_array                 ### Job Name
#SBATCH --array=1-10                                                     ### Number of array tasks
#SBATCH --output=./logs/hla_epitope_registry_eplet_test_array_%A_%a.out  ### File in which to store job output (based on array task)
#SBATCH --error=./logs/hla_epitope_registry_eplet_test_array_%A_%a.err   ### File in which to store job error messages (based on array task)
...
python3 hla_epitope_registry_eplet_test_array.py
