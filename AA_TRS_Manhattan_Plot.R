...
trs_AFA<-read.csv("SRTR_HLA_AA_TRS_Average_AFA.csv")  # Have separate files for each population AFA, ASN, CAU, HIS, MLT, NAM
trs_ASN<-read.csv("SRTR_HLA_AA_TRS_Average_ASN.csv")
trs_CAU<-read.csv("SRTR_HLA_AA_TRS_Average_CAU.csv")
trs_HIS<-read.csv("SRTR_HLA_AA_TRS_Average_HIS.csv")
trs_MLT<-read.csv("SRTR_HLA_AA_TRS_Average_MLT.csv")
trs_NAM<-read.csv("SRTR_HLA_AA_TRS_Average_NAM.csv")
...
manhattan_plot <- function(sub_typ, ethnicity, subj, tolerance=0.075) {
  ...
  don <- sub_typ %>%
    ...
    left_join(sub_typ, ., by=c('Locus'='Locus')) %>%
    
    # Add a cumulative position of each AA position
    arrange(Locus, AA_Position) %>%
  ...
  ggplot(don, aes(x=Loci, y=(1-TRS_Average))) +
    ...
    # Show all points
    geom_point( aes(color=as.factor(Locus)), alpha=0.8, size=1.3) +
